...
#define PCIE_VENDOR_ID 0x666
#define PCIE_DEVICE_ID 0x999
static struct pci_device_id chr_ids[] = {
    {PCIE_VENDOR_ID, PCIE_DEVICE_ID, PCI_ANY_ID,PCI_ANY_ID, 0, 0, 0},
    {0,}
};

MODULE_DEVICE_TABLE(pci,chr_ids);
 
static int chr_probe(struct pci_dev *pdev, const struct pci_device_id *id);
static void chr_remove(struct pci_dev *pdev);
static irqreturn_t chr_interrupt(int irq, void * dev);
...
static void iATU_bar0(void) {
	//下面几步，在手册中有example
	//iATU_write_config_dword(chr_device.pci_dev,iATU Lower Target Address ,xxx);//xxx表示内存中的地址，将bar0映射到这块内存
	//iATU_write_config_dword(chr_device.pci_dev,iATU Upper Target Address ,xxx);//xxx表示内存中的地址，将bar0映射到这块内存
 
	//iATU_write_config_dword(chr_device.pci_dev,iATU Control 1,0x0);//映射的时内存，所以写0x0
	//iATU_write_config_dword(chr_device.pci_dev,iATU Control 2,xxx);//使能某个region，开始地址转换
}
...
void dma_init(void) {
	...
	//1.2 获取msi能力寄存器的地址
	pos =pci_find_capability(chr_device.pci_dev,PCI_CAP_ID_MSI);
	//1.3 读取msi的协议部分，得到pci设备是32位还是64位，不同的架构msi data寄存器地址同
	pci_read_config_word(chr_device.pci_dev,pos+2,&msi_control);
	//1.4 读取msi能力寄存器组中的地址寄存器的值
	pci_read_config_dword(chr_device.pci_dev,pos+4,&msi_addr_l);	
	//1.5 设置 DMA write done IMWr Address Low.这里请根据自己的芯片填写
	//dma_write_config_dword(chr_device.pci_dev,DMA write done IMWr Address Low,msi_addr_l);
	//1.6 设置 DMA write abort IMWr Address Low.这里请根据自己的芯片填写
	//dma_write_config_dword(chr_device.pci_dev,DMA write abort IMWr Address Low,msi_addr_l);
	
	if(msi_control&0x80){
		//64位的
		//1.7 读取msi能力寄存器组中的高32位地址寄存器的值
		pci_read_config_dword(chr_device.pci_dev,pos+0x8,&msi_addr_h);
		//1.8 读取msi能力寄存器组中的数据寄存器的值
		pci_read_config_dword(chr_device.pci_dev,pos+0xc,&msi_data);
		
		//1.9 设置 DMA write done IMWr Address High.这里请根据自己的芯片填写
		//dma_write_config_dword(chr_device.pci_dev,DMA write done IMWr Address High,msi_addr_h);
		//1.10 设置 DMA write abort IMWr Address High.这里请根据自己的芯片填写
		//dma_write_config_dword(chr_device.pci_dev,DMA write abort IMWr Address High,msi_addr_h);
		
	} else {
		//1.11 读取msi能力寄存器组中的数据寄存器的值
		pci_read_config_dword(chr_device.pci_dev,pos+0x8,&msi_data);
	}
	
	//1.12 把数据寄存器的值写入到dma的控制寄存器组中的 DMA write channel 0 IMWr data中
	//dma_write_config_dword(chr_device.pci_dev,DMA write channel 0 IMWr data,msi_data);
	
	//1.13 DMA channel 0 control register 1 = 0x4000010
	//dma_write_config_dword(chr_device.pci_dev,DMA channel 0 control register 1,0x4000010);
...
static int chr_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
	...
	result = request_irq(pdev->irq, chr_interrupt, 0, DEV_NAME, chr_device.pci_dev);
    if (unlikely(result)){
...
static void chr_remove(struct pci_dev *pdev) {
	...
	free_irq(pdev->irq,chr_device.pci_dev);
	pci_disable_msi(pdev);
...
static struct pci_driver chr_driver = {
    .name = DEV_NAME,
    .id_table = chr_ids,
    .probe = chr_probe,
    .remove = chr_remove,
...
static int chr_open(struct inode *inode, struct file *file) {
	printk("driver: chr_open\n");
	//填写产品的逻辑
	return 0;
}
 
int chr_close(struct inode *inode, struct file *file) {
	printk("driver: chr_close\n");
	//填写产品的逻辑
	return 0;
}
 
long chr_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	//填写产品的逻辑
...
static int chr_drv_init(void) {
	int ret;
	ret = pci_register_driver(&chr_driver);
	if (ret < 0) {
		printk("failed: pci_register_driver\n");
		return ret;
	}
	
	ret=alloc_chrdev_region(&chr_device.devno,0,DEVICE_NUMBER,"chr");
	if (ret < 0) {
		printk("failed: register_chrdev_region\n");
		return ret;
	}
 
	cdev_init(&chr_device.cdev, &chr_fops);
	ret = cdev_add(&chr_device.cdev, chr_device.devno, DEVICE_NUMBER);
	if (ret < 0) {
		printk("faield: cdev_add\n");
		return ret;
	}
	
	chr_class = class_create(THIS_MODULE, "chr_class");
	chr_dev = device_create(chr_class, NULL, chr_device.devno, NULL, "chr_device"); 
 
	return 0;
}
 
static void chr_drv_exit(void) {
	device_destroy(chr_class,chr_device.devno);
	class_destroy(chr_class);
		
	cdev_del(&(chr_device.cdev));
	unregister_chrdev_region(chr_device.devno,DEVICE_NUMBER);
	pci_unregister_driver(&chr_driver);
}
